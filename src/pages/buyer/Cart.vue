<template>
  <div class="bg-white pa-3" id="cart">
    ...
    <v-row v-for="cart in cartList" class="align-sm-center position-relative">
      <v-checkbox v-model="isAddOrder[cart.productId]" class="position-absolute top-0 left-0" density="compact" hide-details/>
      <v-btn @click="openDeleteCartDialog(cart.productId, false)" density="comfortable" variant="text" icon="$close"
    ...
    <div class="d-flex flex-column align-end"> 
      ...
      <v-btn @click="handleCheckDialog">前往結帳</v-btn>
    </div>
    <!-- 刪除商品dialog -->
    <v-dialog v-model="dialog.deleteCart">
      <v-card width="300px" class="mx-auto">
        <v-card-text>是否刪除此商品</v-card-text>
        <v-card-actions>
          <v-btn @click="deleteCart">確定</v-btn>
          <v-btn @click="revertQuantity">取消</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- 確定結帳dialog -->
    <v-dialog v-model="dialog.checkout">
      <v-card width="300px" class="mx-auto">
        <v-card-text>是否前往結帳</v-card-text>
        <v-card-actions>
          <v-btn @click="handelCheckConfirm">確定</v-btn>
          <v-btn @click="dialog.checkout = false">取消</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
<script setup lang="ts">
  ...
  const handelCheckConfirm = async (): Promise<void> => {
    const newData = cartList.value.map(cart => {
      let quantity = 0;
