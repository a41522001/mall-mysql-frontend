<template>
  <v-card class="mx-auto" width="100%" max-width="600px">
        ...
        <v-card-text class="pt-5">
          <v-form ref="formAddress">
            <v-row>
              ...
              <v-col cols="6">
                <v-select
                  v-model="receiveAddress.area"
                  :rules="rulesAddress.area"
                  :items="areaList"
                  label="區域"
                  density="compact"
                  variant="outlined"
              ...
              <v-col cols="12">
                <v-text-field
                  ...
                  :rules="rulesAddress.detailAddress"
                  label="詳細地址"
                  density="compact"
                  variant="outlined"
                />
    ...
    <div class="pa-3 text-end">
      <v-btn class="text-white" color="#0D47A1" @click="handlePayment"
        >付款</v-btn
      >
    </div>
  </v-card>
...
const rulesAddress = {
  city: [(v: string) => !!v || "請選擇縣市"],
  area: [(v: string) => !!v || "請選擇區域"],
  detailAddress: [(v: string) => !!v || "請填入詳細地址"],
};
...
const phoneDetail = computed(() => {
  const isCellphone =
    receiverInfo.phone.startsWith("09") && receiverInfo.phone.length === 10;
  if (isCellphone) {
...
const handleValidateReceiver = async (): Promise<
