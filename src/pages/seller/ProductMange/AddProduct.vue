  ...
  <v-card max-width="800" class="mx-auto w-100">
    ...
    <v-card-actions class="d-flex justify-end">
      <v-btn
        color="#E53935"
        variant="elevated"
        @click="clearDataFormAndUploadFile"
        >清除</v-btn
      >
      <v-btn color="#1976D2" variant="elevated" @click="handleAddProduct"
        >儲存商品</v-btn
      >
    </v-card-actions>
  </v-card>
...
const rules = {
  productName: [(v: string) => v.length >= 1 || "請輸入商品名稱"],
  productPrice: [(v: string) => !!v || parseInt(v) > 0 || "請輸入正確的價錢"],
  quantity: [(v: string) => !!v || parseInt(v) > 0 || "請輸入正確的數量"],
};
