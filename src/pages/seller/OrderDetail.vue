<template>
  <div id="orderDetail">
    <div class="mb-5">
      ...
      <div
        class="d-flex flex-column flex-sm-row align-sm-center justify-sm-space-between ga-2"
      >
        <h2>
          訂單詳情:
          <span class="text-indigo-accent-3">{{ orderInfo.orderId }}</span>
        </h2>
        <span
          class="status"
          :class="[statusMap?.[orderInfo.status as Status]?.className]"
        >
    ...
    <div class="mb-5">
      ...
      <template
        v-for="{ productName, price, quantity, image } in orderInfo.products"
      >
        <v-row>
          <v-col cols="3" sm="2">
      ...
      <div class="text-end">
        <div class="mb-2">商品總額： NT$ {{ orderInfo.total }}</div>
        <div class="mb-2">運費： NT$ 0</div>
        <div>
          <span class="text-h5 font-weight-bold">訂單總金額：</span>
          <span class="text-h5 font-weight-bold text-indigo-accent-4"
    ...
    <div>
      ...
      <div class="d-flex ga-3">
        <v-btn
          v-if="displayDeliverCondition(orderInfo.status)"
          color="#1976D2"
          variant="elevated"
        ...
        >
        <v-btn
          v-if="displayCancelCondition(orderInfo.status)"
          color="#E53935"
          variant="elevated"
